...
for i in range(len(x_axis)):
    if y_axis[i].title() in lines.red_line1 and line_id[i] == 1:
        line_conjestion['red_line_1']  += x_axis[i]
    if y_axis[i].title() in lines.yellow_line2 and line_id[i] == 2:
        line_conjestion['yellow_line_2']  += x_axis[i]
    if y_axis[i].title() in lines.blue_line3 and line_id[i] == 3:
        line_conjestion['blue_line_3']  += x_axis[i]
    if y_axis[i].title() in lines.blue_line4 and line_id[i] == 4:
        line_conjestion['blue_line_4']  += x_axis[i]
    if y_axis[i].title() in lines.green_line5 and line_id[i] == 5:
        line_conjestion['green_line_5']  += x_axis[i]
    if y_axis[i].title() in lines.violet_line6 and line_id[i] == 6:
        line_conjestion['violet_line_6']  += x_axis[i]
    if y_axis[i].title() in lines.pink_line7 and line_id[i] == 7:
        line_conjestion['pink_line_7']  += x_axis[i]
    if y_axis[i].title() in lines.magenta_line8 and line_id[i] == 8:
